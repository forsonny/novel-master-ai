...
- End-to-end logs + artifacts stored in .novelmaster/reports/runs/<timestamp-or-id>/.
...
✓ Task 42 complete. PR: https://github.com/org/repo/pull/123
  Run report: .novelmaster/reports/runs/2025-01-15-142033/
```
...
📋 State saved to: .novelmaster/reports/runs/2025-01-15-142033/
    Last error: "POST /api/metrics returns 500 instead of 201"

Next steps:
  - Review diff: git diff HEAD
  - Inspect logs: cat .novelmaster/reports/runs/2025-01-15-142033/log.jsonl
  - Check test output: cat .novelmaster/reports/runs/2025-01-15-142033/test-results/subtask-42.2-green-attempt3.json
...
5) Post‑Run
   ...
   - Persist run report (JSON + markdown summary) to .novelmaster/reports/runs/<run-id>/.
...
- CLI: apps/cli provides command structure and UI components.
  ...
  - New command: tm autopilot (alias: novel-master autopilot).
...
- **Communication**: shared state file `.novelmaster/state/current-run.json` + file watching or event stream
- **Keybindings**:
...
See `.novelmaster/config.json` → `prompts` section for paths and load order.
...
```json
{
  ...
  "git": {
    ...
    "pr": {
      "enabled": true,
      "base": "default",
      "bodyTemplate": ".novelmaster/templates/pr-body.md"
    }
  },
  "prompts": {
...
```
.novelmaster/reports/runs/2025-01-15-142033/
...
- Configurable per project via `.novelmaster/config.json` if too strict
...
4) Where to store persistent TUI state (pane layout, last selection) in .novelmaster/state.json?
...
- Configurable via .novelmaster/config.json: git.branchPattern supports tokens {tag}, {id}, {slug}.
...
- Phase 1: Core Rails (State Machine & Orchestration)
  ...
  - Run report persisted under .novelmaster/reports/runs/ with state checkpoints for resumability.
