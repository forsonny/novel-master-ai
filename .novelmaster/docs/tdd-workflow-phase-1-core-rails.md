...
- **Where:** `~/.novelmaster/projects/<project-path>/runs/<run-id>/`
...
**Philosophy:**
- All runtime state, logs, and throwaway artifacts stored globally in `~/.novelmaster/`
...
function getProjectStoragePath(projectRoot: string): string {
  ...
  return path.join(os.homedir(), '.novelmaster', 'projects', normalized)
  // Example: ~/.novelmaster/projects/-Volumes-Workspace-contrib-novel-master-claude-novel-master
}
```
...
```json
{
  ...
  "projectRoot": "/Volumes/Workspace/contrib/novel-master/claude-novel-master",
  "taskId": "1",
...
└── .gitignore                       # Add: .novelmaster/state/, .novelmaster/reports/
```
...
async function resumeWorkflow(): Promise<void> {
  // 1. Find active run
  const currentRun = await loadJSON('~/.novelmaster/projects/<project>/tags/<tag>/current-run.json')

  // 2. Load state from that run
  const state = await loadJSON(`~/.novelmaster/projects/<project>/runs/${currentRun.runId}/state.json`)
...
**`tm autopilot next`**
- Reads `~/.novelmaster/projects/<project>/tags/<tag>/current-run.json`
- Reads `~/.novelmaster/projects/<project>/runs/<run-id>/state.json`
...
```markdown
You are working autonomously on Novel Master tasks using the autopilot workflow.
...
**Add to `.novelmaster/config.json` (versioned):**
```json
{
  "autopilot": {
    "enabled": true,
...
- Verify global storage created in `~/.novelmaster/projects/<project>/`
