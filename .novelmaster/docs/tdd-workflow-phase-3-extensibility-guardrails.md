...
class AutopilotTUI {
  ...
  private watchStateFile() {
    watch('.novelmaster/state/current-run.json', (event, filename) => {
      this.updateDisplay()
    })
  }
...
**State File** (`.novelmaster/state/current-run.json`):
```json
{
...
- `.novelmaster/state/current-run.json` - live run state
- `.novelmaster/reports/runs/<runId>/manifest.json` - run metadata
- `.novelmaster/reports/runs/<runId>/log.jsonl` - event stream
...
```json
{
  "autopilot": {
    ...
    "tui": {
      "enabled": false,
      "tmuxSession": "novelmaster-autopilot"
    },
    "api": {
      "enabled": false,
      "port": 7890,
