...
Novel Master uses two primary configuration methods:

1.  **`.novelmasterconfig` File (Project Root)**
    ...
    - **Created and managed using `novel-master models --setup` CLI command or the `models` MCP tool.**
...
2.  **Environment Variables (`.env` file or MCP `env` block)**
    ...
    - **For MCP/Cursor:** Place keys in the `env` section of your `.cursor/mcp.json` (or other MCP config according to the AI IDE or client you use) file under the `novelmaster-ai` server definition.

**Important:** Settings like model choices, max tokens, temperature, and log level are **no longer configured via environment variables.** Use the `novel-master models` command or tool.
...
1. **ES Modules Configuration:**
   ...
   - If you need CommonJS throughout your project, remove `"type": "module"` from package.json, but Novel Master scripts expect ESM.
...
novel-master parse-prd your-prd.txt
...
Claude Novel Master is designed to work seamlessly with [Cursor AI](https://www.cursor.so/), providing a structured workflow for AI-driven development.
...
Once configured, you can interact with Novel Master's task management commands directly through Cursor's interface, providing a more integrated experience.
...
```
Please use the novel-master parse-prd command to generate tasks from my PRD. The PRD is located at scripts/prd.txt.
```

The agent will execute:

```bash
novel-master parse-prd scripts/prd.txt
```
...
- Run `novel-master list` to see all tasks
- Run `novel-master next` to determine the next task to work on
...
```bash
novel-master update --from=4 --prompt="Now we are using Express instead of Fastify."
```
...
```bash
novel-master expand --id=5 --num=3
```
...
```bash
novel-master expand --id=5 --prompt="Focus on security aspects"
```
...
```
I've just initialized a new project with Claude Novel Master. I have a PRD at scripts/prd.txt.
