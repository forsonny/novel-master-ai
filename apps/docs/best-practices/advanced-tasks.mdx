...
- Run `novel-master show 1,3,5` to display multiple tasks with interactive options
...
```bash
novel-master update --from=4 --prompt="Now we are using MongoDB instead of PostgreSQL."

# OR, if research is needed to find best practices for MongoDB:
novel-master update --from=4 --prompt="Update to use MongoDB, researching best practices" --research
```
...
- Moving multiple tasks at once: `--from=10,11,12 --to=16,17,18` (must have same number of IDs, Novel Master will look through each position)
