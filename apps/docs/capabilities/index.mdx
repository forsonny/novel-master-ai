...
Discover the technical capabilities of Novel Master, including supported models, integrations, and more. 

# CLI Interface Synopsis

This document outlines the command-line interface (CLI) for the Novel Master application, as defined in `bin/novel-master.js` and the `scripts/modules/commands.js` file (which I will assume exists based on the context). This guide is intended for those writing user-facing documentation to understand how users interact with the application from the command line.

## Entry Point

The main entry point for the CLI is the `novel-master` command, which is an executable script that spawns the main application logic in `scripts/dev.js`.
...
-   **`parse-prd`**: Parses an NRD (Novel Requirements Document) to generate story arcs and chapters.
    -   `--file <file>`: The path to the NRD file.
...
This document provides a high-level overview of the core implementation of the Novel Master application, focusing on the functionalities exposed through `scripts/modules/task-manager.js`. This serves as a guide for understanding the application's capabilities when writing user-facing documentation.
...
-   **`parsePRD(prdPath, numTasks)`**: Parses a Novel Requirements Document (NRD) to generate an initial set of story arcs and chapters.
...
-   **`migrateProject()`**: Migrates the project to the new `.novelmaster` directory structure.
...
This document provides an overview of the MCP (Machine-to-Machine Communication Protocol) interface for the Novel Master application. The MCP interface is defined in the `mcp-server/` directory and exposes the application's core functionalities as a set of tools that can be called remotely.

## Core Concepts

The MCP interface is built on top of the `fastmcp` library and registers a set of tools that correspond to the core functionalities of the Novel Master application. These tools are defined in the `mcp-server/src/tools/` directory and are registered with the MCP server in `mcp-server/src/tools/index.js`.
...
-   **`parse_prd`**: Parses an NRD (Novel Requirements Document) to generate story arcs and chapters.
...
This synopsis provides a clear overview of the MCP interface and its available tools, which will be valuable for anyone writing documentation for developers who need to interact with the Novel Master application programmatically.