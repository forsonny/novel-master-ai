...
To optimize LLM context usage, you can control which Novel Master MCP tools are loaded using the `NOVEL_MASTER_TOOLS` environment variable. This is particularly useful when working with LLMs that have context limits or when you only need a subset of tools.

### Configuration Modes

#### All Tools (Default)
Loads all 36 available tools. Use when you need full Novel Master functionality.
...
If `NOVEL_MASTER_TOOLS` is not set, all tools are loaded by default.
...
- `parse_prd` - Generate story arcs and chapters from NRD (Novel Requirements Document)
...
```json
{
  "mcpServers": {
    "novel-master-ai": {
      ...
      "args": ["-y", "novel-master-ai"],
      "env": {
        "NOVEL_MASTER_TOOLS": "get_tasks,next_task,set_task_status,update_subtask",
        "ANTHROPIC_API_KEY": "your_key_here"
      }
    }
  }
...
```
Novel Master MCP Server starting...
