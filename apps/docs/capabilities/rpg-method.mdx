---
title: RPG Method for NRD Creation
sidebarTitle: "RPG Method"
---

# Repository Planning Graph (RPG) Method

The RPG (Repository Planning Graph) method is an advanced approach to creating Novel Requirements Documents that generate highly-structured, dependency-aware story graphs. It's based on Microsoft Research's methodology for scalable narrative structure generation.

## When to Use RPG

Use the RPG template (`example_nrd_epic.md`) for:

- **Complex multi-arc stories** with intricate dependencies
- **Large-scale novels** being planned from scratch
- **Stories requiring explicit structure** and clear arc boundaries
- **Teams needing dependency visibility** for parallel writing

For simpler stories or smaller projects, the standard `example_nrd.md` template may be more appropriate.
...
Novel Master automatically orders tasks based on this dependency chain.
...
## Using RPG with Novel Master

### Step 1: Create NRD with Epic Template

Use a narrative-context-aware tool to fill out the template:

```bash
# In Claude Code, Cursor, or similar
"Create an NRD using @.novelmaster/templates/example_nrd_epic.md for [your story]"
```

**Why narrative context matters:** The AI needs to understand your existing story structure to make informed decisions about:
- Story arc boundaries
- Narrative dependency relationships
- Character arc integration points
- POV and timeline conventions
...
### Step 2: Parse NRD into Tasks

```bash
novel-master parse-prd .novelmaster/docs/your-story.md --research
```

Novel Master will:
1. Extract story arcs → Main tasks (chapters)
2. Extract scenes → Subtasks (beats)
3. Parse dependencies → Narrative dependencies
...
- Onboarding aid (structural overview in NRD)
...
4. Let `novel-master expand` break down complex tasks further

### 4. Use Research Mode
```bash
novel-master parse-prd --research
```
...
2. **Reference epic template**: Show AI the `example_nrd_epic.md`
3. **Co-create NRD**: Work through each section with AI guidance
4. **Save to docs**: Place in `.novelmaster/docs/your-story.md` (use `.md` for better editor support)
5. **Parse NRD**: `novel-master parse-prd .novelmaster/docs/your-story.md --research`
6. **Analyze**: `novel-master analyze-complexity --research`
7. **Expand**: `novel-master expand --all --research`
8. **Start work**: `novel-master next`

---

## Further Reading

- [NRD Creation and Parsing Guide](/getting-started/quick-start/prd-quick)
