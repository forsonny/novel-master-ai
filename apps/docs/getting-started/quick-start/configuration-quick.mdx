...
Before getting started with Novel Master, you'll need to set up your API keys. There are a couple of ways to do this depending on whether you're using the CLI or working inside MCP. It's also a good time to start getting familiar with the other configuration options available — even if you don’t need to adjust them yet, knowing what’s possible will help down the line.

## API Key Setup

Novel Master uses environment variables to securely store provider API keys and optional endpoint URLs.
...
<Tip>
**Optimize Context Usage**: You can control which Novel Master MCP tools are loaded using the `NOVEL_MASTER_TOOLS` environment variable. This helps reduce LLM context usage by only loading the tools you need.
...
The main configuration file (`.novelmaster/config.json`) allows you to control nearly every aspect of Novel Master’s behavior. Here’s a high-level look at what you can customize:
