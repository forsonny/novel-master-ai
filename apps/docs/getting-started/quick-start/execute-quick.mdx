...
Novel Master has the "next" command to find the next task to work on. You can access it with the following request:
```
...
```bash
novel-master set-status --id=5 --status=done
```
...
By now you have all you need to get started executing code faster and smarter with Novel Master.
