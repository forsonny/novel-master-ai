...
```bash
novel-master update-task --id=2 --prompt="use Postgres instead of MongoDB and remove the sharding subtask"
```
## Analyze complexity

Novel Master can provide a complexity report which can be helpful to read before you begin. If you didn't already expand all your tasks, it could help identify which could be broken down further with subtasks.
...
<Tip>
The `--research` flag uses whatever research model you have configured in `.novelmaster/config.json` (configurable via `novel-master models --setup`) for research-backed complexity analysis, providing more informed recommendations.
</Tip>
