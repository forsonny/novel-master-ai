...
> **Note:** This file works alongside `AGENTS.md` (generic AI agent instructions). AGENTS.md contains the core Novel Master commands and workflows for all AI agents. This file contains only Gemini CLI-specific features and integrations.

## MCP Configuration for Gemini CLI

Configure Novel Master MCP server in `~/.gemini/settings.json`:
...
**Note:** API keys are configured via `novel-master models --setup`, not in MCP configuration.
...
novel-master models --set-main gemini-2.0-flash-exp
novel-master models --set-fallback gemini-1.5-flash
...
As a Gemini CLI assistant with Novel Master:
...
**Key Principle:** Focus on natural conversation. Novel Master MCP tools work seamlessly with Gemini CLI's interface.

---

*See AGENTS.md for complete Novel Master commands, workflows, and best practices.*
