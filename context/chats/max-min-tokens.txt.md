We want to refine how Novel Master handles AI model token limits to be more precise, by:
...
1.  **Update `.novelmasterconfig` Structure:**
    - I'll modify the `models` section in your `.novelmasterconfig`. For each role (`main`, `research`, `fallback`), `maxTokens` will be replaced with `maxInputTokens` and `maxOutputTokens`.
...
2.  **Update `MODEL_MAP` in `ai-services-unified.js`:**
    - This array already stores cost data. We need to ensure it also stores the _absolute_ maximum input and output tokens for each model listed (e.g., `model_max_input_tokens`, `model_max_output_tokens`). If these fields are not present, they will need to be added. The values in `.novelmasterconfig` will then represent user-defined operational limits, which should ideally be validated against these absolute maximums.
...
2.  **Validation:** Add validation (perhaps in `config-manager.js` or when `novel-master models --setup` is run) to ensure `configured_max_input_tokens` and `configured_max_output_tokens` in `.novelmasterconfig` do not exceed the `model_absolute_max_input_tokens` and `model_absolute_max_output_tokens` from `MODEL_MAP`.

Let's start by inspecting `scripts/modules/config-manager.js` to see how parameters and `MODEL_MAP` are currently handled. This will inform the changes needed for `.novelmasterconfig` and `ai-services-unified.js`.
...
- **Configuration Loading:** `_loadAndValidateConfig` handles loading `.novelmasterconfig` and merging it with `DEFAULTS`.
...
2.  **Update `.novelmasterconfig` (Defaults and User File):**
    ...
    - You will then need to manually update your existing `.novelmasterconfig` file to reflect this new structure. I can provide the snippet for you to paste.
3.  **Update `config-manager.js`:**
    ...
    - (Optional, for later validation) Consider adding a new function `getModelCapabilities(providerName, modelId)` that reads from `MODEL_MAP` to return the absolute `maxInputTokens` and `maxOutputTokens` for a given model. This would be useful for validating the user's settings in `.novelmasterconfig`.
...
Once I have the content of `supported-models.json`, I can draft the changes for it and then proceed to `config-manager.js` and `.novelmasterconfig`.
...
// For a model like Anthropic Claude 3.7 Sonnet (Preview)
{
    ...
    "maxOutputTokens": 8192     // Anthropic default, user can override in .novelmasterconfig
}
...
1.  Updating `.novelmasterconfig` defaults in `config-manager.js` and providing you the snippet for your user file.
