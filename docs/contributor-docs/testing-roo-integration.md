...
This document provides instructions for testing the Roo integration in the Novel Master package.
...
3. Install the novel-master-ai package locally:

   ```bash
   # From the root of the claude-novel-master repository
   cd ..
   npm pack
   # This will create a file like novel-master-ai-0.12.0.tgz

   # Move back to the test directory
   cd test-tm
   npm install ../novel-master-ai-0.12.0.tgz
   ```

4. Initialize a new Novel Master project:

   ```bash
   npx novel-master init --yes
   ```
