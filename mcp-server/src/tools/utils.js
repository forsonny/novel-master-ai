/**
 * tools/utils.js
 * Utility functions for Novel Master CLI integration
 */
...
function getProjectRoot(projectRootRaw, log) {
	// PRECEDENCE ORDER:
	// 1. Environment variable override (NOVEL_MASTER_PROJECT_ROOT)
	...
	if (envRoot) {
		...
		log.info(
			`Using project root from NOVEL_MASTER_PROJECT_ROOT environment variable: ${absolutePath}`
		);
		return absolutePath;
	}
	...
	// 4. Check if the current directory has any indicators of being a novel-master project
	...
	log.warn(
		`No novel-master project detected in current directory. Using ${currentDir} as project root.`
	);
	log.warn(
		'Consider using --project-root to specify the correct project location or set NOVEL_MASTER_PROJECT_ROOT environment variable.'
	);
	return currentDir;
}
...
/**
 * Executes a novel-master CLI command synchronously.
 ...
 */
function executeNovelMasterCommand(
	command,
	log,
	args = [],
	projectRootRaw = null,
	...
	try {
		...
		log.info(
			`Executing novel-master ${command} with args: ${JSON.stringify(
				args
			)} in directory: ${cwd}`
		);
		...
		// Execute the command using the global novel-master CLI or local script
		// Try the global CLI first
		let result = spawnSync('novel-master', fullArgs, spawnOptions);

		// If global CLI is not available, try fallback to the local script
		if (result.error && result.error.code === 'ENOENT') {
			log.info('Global novel-master not found, falling back to local script');
		...
		log.error(`Error executing novel-master command: ${error.message}`);
		return {
			success: false,
			error: error.message
		};
 ...
 * Uses NOVEL_MASTER_PROJECT_ROOT environment variable with proper precedence.
