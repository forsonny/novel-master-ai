...
export class TaskLoaderService {
	...
	async loadAndValidateTask(taskId: string): Promise<TaskValidationResult> {
		...
		if (!task) {
			return {
				...
				suggestion:
					'Use "novel-master list" to see available tasks or verify the task ID is correct.'
			};
		}
	...
	private buildExpansionSuggestion(task: Task): string {
		...
		suggestions.push(`  novel-master expand --id=${task.id}`);
		...
		if (task.complexity || task.recommendedSubtasks) {
			...
			suggestions.push(`  novel-master show ${task.id}`);
		} else {
			suggestions.push(
				`\nOr analyze task complexity first to determine optimal subtask count:`
			);
			suggestions.push(`  novel-master analyze-complexity --from=${task.id}`);
		}
