...
function enhanceRooMCPConfiguration(mcpPath) {
	...
	try {
		...
		if (mcpConfig.mcpServers && mcpConfig.mcpServers['novel-master-ai']) {
			const server = mcpConfig.mcpServers['novel-master-ai'];
			...
			log('warn', `[Roo] novel-master-ai server not found in MCP configuration`);
		}
	} catch (error) {
