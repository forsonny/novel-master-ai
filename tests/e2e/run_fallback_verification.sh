...
jq -c 'to_entries[] | .key as $provider | .value[] | select(.allowed_roles[]? == "fallback") | {provider: $provider, id: .id}' "$SUPPORTED_MODELS_FILE" | while IFS= read -r model_info; do
    ...
    if ! command -v novel-master &> /dev/null; then
        log_error "novel-master command not found."
        echo "[INSTRUCTION FV] Please run 'npm link novel-master-ai' in the project root first."
        exit 1
    fi
    log_info "Setting main model to $model_id ${flag:+using flag $flag}..."
    set_model_cmd="novel-master models --set-main \"$model_id\" $flag"
    ...
    timeout 120s novel-master update-subtask --id=1.1 --prompt="Simple test prompt to verify generateObjectService call." 2>&1 | tee "$update_subtask_output_file" &
    # Store the command output in a variable simultaneously
    # update_subtask_command_output=$(timeout 120s novel-master update-subtask --id=1.1 --prompt="Simple test prompt to verify generateObjectService call." 2>&1)
...
echo ""
echo "Test Command: novel-master update-subtask --id=1.1 --prompt=\"...\" (tests generateObjectService)"
