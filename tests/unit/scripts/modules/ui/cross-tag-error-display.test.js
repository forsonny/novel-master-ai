...
describe('Cross-Tag Error Display Functions', () => {
	...
	describe('displayDependencyValidationHints', () => {
		it('should display general hints by default', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('💡 Use "novel-master validate-dependencies"')
			);
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('💡 Use "novel-master fix-dependencies"')
			);
		});
		...
		it('should display before-move hints', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining(
					'💡 Tip: Run "novel-master validate-dependencies"'
				)
			);
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining('💡 Tip: Use "novel-master fix-dependencies"')
			);
		});
		...
		it('should display after-error hints', () => {
			...
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining(
					'🔧 Quick fix: Run "novel-master validate-dependencies"'
				)
			);
			expect(mockConsoleLog).toHaveBeenCalledWith(
				expect.stringContaining(
					'🔧 Quick fix: Use "novel-master fix-dependencies"'
				)
			);
		});
